...
// ⚠⚠ UN ANNEAU NE SE COUPE JAMAIS PAR SLICE — correctif du 03/09/2026.
// L'ancien code plafonnait le contour par `reduit.slice(0, 40)`. Couper la queue
// d'un anneau l'OUVRE : PAINT le referme par une CORDE entre le 40e sommet et le
// premier, et tout le lobe laissé de l'autre côté de cette corde reste BLANC sur
// le plan. C'est la cause exacte des « parties de parcelles non colorisées ».
// Deux garanties désormais : on desserre la tolérance jusqu'à tenir dans le
// budget, et si cela ne suffit pas on RÉÉCHANTILLONNE uniformément (ce qui
// conserve le tour complet), puis on REFERME explicitement l'anneau.
const fermerAnneau = (pts) => {
...
const reduireAnneau = (pts, max, tol0) => {
  ...
  if (r.length > max || r.length < 4) {
    ...
    const n = Math.max(4, Math.min(max - 1, uniques));
    r = Array.from({ length: n }, (_, k) => pts[Math.floor(k * uniques / n)]);
  }
  return fermerAnneau(r);
};

// Anneaux EXTÉRIEURS d'une géométrie. ⚠ anneauxDe (plus bas) renvoie AUSSI les
// trous : y choisir « l'anneau le plus long » pouvait peindre un TROU à la place
// du fonds. Et une parcelle en PLUSIEURS MORCEAUX (MultiPolygon) n'était
// coloriée que sur son plus gros morceau, le reste restant blanc en silence.
const anneauxExterieursDe = (geom) => {
  if (!geom) return [];
  if (geom.type === 'Polygon') return geom.coordinates.slice(0, 1);
  if (geom.type === 'MultiPolygon') return geom.coordinates.map((q) => q[0]).filter(Boolean);
  return [];
};

// UN BLOC = les anneaux d'UNE parcelle, séparés par « ~ ». Les parcelles, elles,
// se séparent par « | ». Contrat d'URL étendu le 03/09/2026, côté PAINT aussi.
const blocPoly = (anneaux) => (anneaux || [])
  .filter(Boolean)
  .map((pts) => pts.map(([X, Y]) => `${X.toFixed(1)},${Y.toFixed(1)}`).join(';'))
  .join('~');
...
const descripteursForme = (geom, dep) => {
  ...
  let poly = null, polys = null;
  if (cc) {
    const projetesParAnneau = [];
    for (const a of anneaux) {
      const projetes = [];
      let ok = true;
      for (const [lo, la] of a) {
        const q = versConiqueConforme(la, lo, dep);
        if (!q || q.zone !== cc.zone) { ok = false; break; }
    ...
    if (rendus.length) {
      ...
      // anciens appelants raisonnent sur une seule forme.
      const principal = rendus.find((r) => r.src === anneau) || rendus[0];
      poly = principal.pts;
    }
  }
  return {
...
const lienPaintUnite = (membres, parParcelle, contours, nomCommune, adressePar) => {
  ...
  // ⚠⚠ UN EMPLACEMENT PAR MEMBRE, contour ou non — correctif du 03/09/2026.
  // PAINT apparie poly, pp et tab PAR POSITION. L'ancien `filter(Boolean)`
  // retirait ici les parcelles sans contour (2,7 % du relevé) alors que
  // suffixePP et tab les conservent : à partir du premier trou, chaque parcelle
  // recevait le POLYGONE de la précédente — donc un polygone hors cadre, un
  // overlay vide et AUCUNE colorisation — et la dernière parcelle tombait du
  // document, la boucle de PAINT tournant blocs.length fois. Tout cela en
  // silence. Un membre sans contour émet désormais un bloc VIDE, qui garde sa
  // place ; PAINT ne filtre plus les blocs vides et sort un extrait brut marqué.
  const projetes = [];
  for (const ref of membres) {
    const g = contours.get(ref);
    const exts = g ? anneauxExterieursDe(g) : [];
    if (!exts.length) { projetes.push(null); continue; }
    const anneaux = [];
    for (const ext of exts) {
      ...
      if (ok && pts.length >= 4) anneaux.push(pts);
    }
    projetes.push(anneaux.length ? anneaux : null);
  }
  if (projetes.filter(Boolean).length < 2) return null;
...
const grouperPourCarte = (liste) => {
  ...
  for (const o of liste || []) {
    ...
    c.nb += 1;
    // Une ASSIETTE de copropriété (la société n'y détient que des lots) compte
    // comme parcelle à dessiner, pas comme surface détenue (v11 § 5).
    c.surface += o._assiette ? 0 : m2;
    ...
    s.lignes.push({ ref: r, numero: sn ? sn.numero : '', m2, adresse: o.adresse || '', assiette: !!o._assiette });
    s.surface += o._assiette ? 0 : m2;
  }
...
const lienPaintColorise = (codeParcelle, nomCommune, geom, annotations) => {
  ...
  if (f) {
    ...
    if (f.cc) {
      ...
      if (f.poly) {
        qs.set('poly', blocPoly(f.polys || [f.poly]));
      }
    }
  }
...
const suffixeBati = (refs, batiPar) => {
  ...
  refs.forEach((r) => {
    ...
    if (!e) return;
    const sn = sectionEtNumero(r);
    // ⚠ LE BÂTIMENT EST UN CODE RELATIF À SA PARCELLE — correctif du 03/09/2026.
    // « A » sur AI 576 et « A » sur 248 AB 617 sont DEUX bâtiments distincts.
    // L'ensemble de simples lettres les confondait : DUNKERQUE annonçait
    // « 17 bâtiments » (les lettres A à Q) pour 27 bâtiments réels.
    e.bats.forEach((b) => bats.add(r + '|' + b));
...
export default function App() {
  ...
  // 'indisponible' | 'erreur'), syndicat, unité foncière reconstituée.
  const [assiettesInfo, setAssiettesInfo] = useState(null);
  // Contours des parcelles du syndicat, chargés à part : ils n'appartiennent
  // pas au portefeuille et ne doivent pas entrer dans unitesF ni la cohérence.
  const [contoursAssiette, setContoursAssiette] = useState(null);
  ...
  // ----------------------------------------------------------------------
  // PARCELLES D'ASSIETTE — 03/09/2026 (addendum v11 § 5)
  // Une société qui ne détient que des LOTS de copropriété n'a aucune ligne au
  // fichier des parcelles : le sol n'est pas à son compte. Mais le fichier des
  // LOCAUX rattache chaque lot à sa parcelle. Ces parcelles — les assiettes —
  // alimentent désormais la chaîne des plans (panier, Colorier, Document,
  // Dossier complet), MARQUÉES comme assiettes, avec la contenance du PLAN
  // (la matrice ne la donne pas à cette société), et EXCLUES de la surface du
  // portefeuille, qui reste celle du foncier détenu.
  // ----------------------------------------------------------------------
  const refsNonBati = new Set(parcelles.map((o) => o.codeParcelle).filter(Boolean));
  const assiettes = (() => {
    const m = new Map();
    locaux.forEach((o) => {
      const r = o.codeParcelle;
      if (!r || refsNonBati.has(r)) return;
      if (!m.has(r)) {
        m.set(r, {
          codeParcelle: r, codeInsee: o.codeInsee, commune: o.commune,
          departement: o.departement, codeDepartement: o.codeDepartement, region: o.region,
          adresse: o.adresse || '',
          contenance: o.contenanceCadastre ?? null, contenanceCadastre: o.contenanceCadastre ?? null,
          coordonnees: o.coordonnees || null, codeDroit: o.codeDroit,
          _assiette: true, _planLu: o._planLu, _absenteDuPlan: o._absenteDuPlan, nbLots: 0,
        });
      }
      m.get(r).nbLots += 1;
    });
  ...
  // Assiette ENTIÈRE reconstituée par le syndicat (v11 § 5 c) : les parcelles
  // du syndicat situées dans la même unité foncière que l'assiette du lot,
  // hors celles déjà connues. Une parcelle du syndicat détachée (parking à
  // 300 m) n'entre pas : ce n'est pas la même unité.
  const assiettesEtendues = (() => {
    if (!assiettesInfo) return [];
    const deja = new Set([...refsNonBati, ...assiettes.map((a) => a.codeParcelle)]);
    const m = new Map();
    assiettesInfo.forEach((info, r0) => {
      (info.unite || []).forEach((o) => {
        if (!o.codeParcelle || deja.has(o.codeParcelle) || m.has(o.codeParcelle)) return;
        m.set(o.codeParcelle, { ...o, _assiette: true, _viaSyndicat: info.syndicat ? info.syndicat.siren : null, _assietteDe: r0, nbLots: 0 });
      });
    });
    return [...m.values()];
  })();
  ...
  const contoursPlan = (contours || contoursAssiette)
    ? new Map([...(contours || []), ...(contoursAssiette || [])]) : null;
  ...
  // ----------------------------------------------------------------------
  // SYNDICATS DE COPROPRIÉTAIRES — 03/09/2026 (addendum v11 § 5 c-d)
  // Pour chaque parcelle d'assiette (un lot de la société, pas de ligne au
  // fichier des parcelles) : QUI DÉTIENT LE SOL ? → /api/proprietaires (index
  // inverse). Si c'est un syndicat (groupe de personne 7) avec un vrai SIREN,
  // on relit ses parcelles par le chemin NORMAL (/api/parcelles), on charge
  // leurs contours, et calculerUnites donne l'unité foncière qui contient
  // l'assiette du lot : c'est l'assiette ENTIÈRE de la copropriété. Une
  // parcelle du syndicat détachée reste dehors, ce qui est juste.
  // États rendus, tous DITS à l'écran et à l'Excel :
  //   syndicat        assiette reconstituée (SIREN)
  //   majic           syndicat sans SIREN réel : identifié, non reconstitué
  //                   (le balayage des voisines par n° MAJIC est un chantier)
  //   autre_titulaire le sol est à une autre personne morale (bailleur…)
  //   non_recense     aucune personne morale au sol : copropriétaires
  //                   personnes physiques — le cas le plus fréquent
  //   indisponible    index inverse absent de la release FPMU (< 2026)
  //   erreur          appel en échec
  // ----------------------------------------------------------------------
  const rechercherSyndicats = async (refsAssiette, listeLocaux, geometries) => {
    const info = new Map();
    const communeDe = (r) => {
      const l = listeLocaux.find((o) => o.codeParcelle === r);
      return l ? { codeInsee: l.codeInsee, commune: l.commune } : {};
    };
    refsAssiette.forEach((r) => info.set(r, { etat: 'recherche', ...communeDe(r) }));
    setAssiettesInfo(new Map(info));

    // 1. Titulaires du sol, par lots de 400.
    const titulairesPar = new Map();
    let indisponible = null;
    for (let i = 0; i < refsAssiette.length && !indisponible; i += 400) {
      const lot = refsAssiette.slice(i, i + 400);
      try {
        const r = await fetch(`${BACKEND_URL}/api/proprietaires?ids=${lot.join(',')}`);
        if (r.status === 503) {
          const e = await r.json().catch(() => ({}));
          indisponible = e.erreur || 'Index inverse indisponible sur cette release FPMU.';
          break;
        }
        if (!r.ok) { const e = await r.json().catch(() => ({})); throw new Error(e.erreur || e.error || `HTTP ${r.status}`); }
        const d = await r.json();
        (d.results || []).forEach((p) => titulairesPar.set(p.code_parcelle, p));
        (d.manquants || []).forEach((c) => titulairesPar.set(c, null));
      } catch (e) {
        lot.forEach((c) => info.set(c, { ...info.get(c), etat: 'erreur', motif: e.message }));
      }
    }
    if (indisponible) {
      refsAssiette.forEach((r) => info.set(r, { ...info.get(r), etat: 'indisponible', motif: indisponible }));
      setAssiettesInfo(new Map(info));
      return;
    }
    ...
    refsAssiette.forEach((r) => {
      const cur = info.get(r);
      if (cur.etat === 'erreur') return;
      const p = titulairesPar.get(r);
      if (!p) { info.set(r, { ...cur, etat: 'non_recense' }); return; }
      const tit = p.titulaires || [];
      // Le syndicat reconnu par le GROUPE de personne l'emporte sur celui reconnu
      // par sa dénomination ; à égalité, celui qui a un identifiant recherchable.
      const rang = (t) => (t.syndicat_par === 'groupe' ? 0 : 1) + (t.siren_recherchable ? 0 : 2);
      const synd = tit.filter((t) => t.syndicat).sort((a, b) => rang(a) - rang(b))[0];
      if (!synd) { info.set(r, { ...cur, etat: 'autre_titulaire', titulaires: tit, contenanceSol: p.contenance }); return; }
      // ⚠ Un identifiant FICTIF MAJIC (U21440362) est recherchable comme un vrai
      // SIREN : les parquets sont triés sur le texte (§ 5 f). Constaté à
      // Dunkerque le 03/09/2026 : « SYNDICAT DES COPROPRIETAIRES LA BATELLERIE »,
      // groupe 0, U21440362, DEUX parcelles sous le même identifiant.
      const sy = { siren: synd.numero_siren, denomination: synd.denomination, majic: synd.numero_majic,
                   par: synd.syndicat_par, sirenReel: !!synd.siren_reel, recherchable: !!synd.siren_recherchable };
      if (sy.recherchable) {
        info.set(r, { ...cur, etat: 'syndicat', syndicat: sy, titulaires: tit, contenanceSol: p.contenance });
        if (!parSyndicat.has(sy.siren)) parSyndicat.set(sy.siren, []);
        parSyndicat.get(sy.siren).push(r);
      } else {
        info.set(r, { ...cur, etat: 'majic', syndicat: sy, titulaires: tit, contenanceSol: p.contenance });
      }
    });
    setAssiettesInfo(new Map(info));

    // 3. Parcelles de chaque syndicat, contours, unité foncière de l'assiette.
    const geoms = new Map();
    for (const [siren, refs] of parSyndicat) {
      try {
        const r = await fetch(`${BACKEND_URL}/api/parcelles?siren=${encodeURIComponent(siren)}&maxResults=2000`);
        if (!r.ok) { const e = await r.json().catch(() => ({})); throw new Error(e.erreur || e.error || `HTTP ${r.status}`); }
        const d = await r.json();
        ...
        for (const [insee, set] of parCommune) {
          const ids = [...set];
          for (let i = 0; i < ids.length; i += 400) {
            ...
            } catch { /* une commune absente du plan ne bloque pas les autres */ }
          }
        }
        // L'assiette du lot a déjà son contour dans le géocodage du relevé.
        refs.forEach((r0) => { const c = geometries && geometries.get(r0); if (c && !geoms.has(r0)) geoms.set(r0, c); });
        const u = calculerUnites(liste, geoms);
        refs.forEach((r0) => {
          const cur = info.get(r0);
          const n = u ? u.numero.get(r0) : null;
          const membres = n ? u.unites[n - 1].membres : null;
          const vus = new Set();
          const unite = membres
            ? liste.filter((o) => membres.includes(o.codeParcelle) && !vus.has(o.codeParcelle) && vus.add(o.codeParcelle))
            : [];
          info.set(r0, {
            ...cur,
            parcellesSyndicat: liste.length,
            totalSyndicat: Number(d.total) || liste.length,
            uniteTrouvee: !!membres,
            unite,
            // Sans contour sur l'assiette, l'unité ne peut pas être calculée :
            // on le dit, on ne devine pas.
            motif: membres ? null : (geoms.has(r0)
              ? 'assiette isolée dans le portefeuille du syndicat'
              : 'contour de l\'assiette absent du plan : unité non calculable'),
          });
        });
      } catch (e) {
        refs.forEach((r0) => info.set(r0, { ...info.get(r0), motif: `parcelles du syndicat : ${e.message}` }));
      }
    }
    setContoursAssiette(new Map(geoms));
    setAssiettesInfo(new Map(info));
  };
  ...
  const geocoderTout = async (listeParcelles, listeLocaux) => {
    ...
    const refsNB = new Set(listeParcelles.map((o) => o.codeParcelle).filter(Boolean));
  ...
  const surfaceParRef = (() => {
    const m = new Map();
    // parcellesPlan et non parcelles : les assiettes portent la contenance du
    // plan, nécessaire au tableau de désignation de PAINT.
  ...
  const adresseParRef = (() => {
    const m = new Map();
    // Le marqueur d'assiette voyage dans le 4e champ de « tab » (colonne
    // Lieudit de la désignation PAINT) : le lecteur du document voit que la
  ...
  const carteGroupes = carteOuverte ? grouperPourCarte(parcellesPlan) : null;
  const carteCommuneObj = (carteGroupes && carteCommune) ? carteGroupes.find((c) => c.insee === carteCommune) : null;
  const carteRefs = [...carteSel];
  const carteApercu = (carteOuverte && carteRefs.length) ? mesurerSelection(carteRefs, contoursPlan) : null;
  // Les assiettes ne comptent pas dans la surface : la société n'en détient pas le sol.
  const carteSurface = carteRefs.reduce((t, r) => t + (assietteRefs.has(r) ? 0 : (Number(surfaceParRef.get(r)) || 0)), 0);
  ...
  const ouvrirCarte = () => {
    ...
    setQCarteCommune('');
    const g = grouperPourCarte(parcellesPlan);
  ...
  const genererCarte = (mode = 'plan') => {
    ...
    if (!lien) {
      ...
      lien = lienPaintAnnote(pivot, nom, contoursPlan?.get(pivot), surfaceParRef.get(pivot), autres, adresseParRef.get(pivot));
    }
    if (!lien) return;
  ...
  const dossierLignes = dossierOuvert
    ? grouperPourCarte(parcellesPlan).slice().sort((a, b) => a.nom.localeCompare(b.nom, 'fr'))
        .flatMap((c) => {
          ...
          return vols.map((refs, i) => ({
            ...
            refs, nb: refs.length,
            nbAssiettes: refs.filter((r) => assietteRefs.has(r)).length,
            // Surface du foncier DÉTENU : les assiettes n'y entrent pas.
            surface: refs.reduce((t, r) => t + (assietteRefs.has(r) ? 0 : (Number(surfaceParRef.get(r)) || 0)), 0),
          }));
        })
    : null;
  const lienDossierCommune = (lg) => {
    ...
    if (!lien) {
      ...
      lien = lienPaintAnnote(pivot, lg.nom, contoursPlan?.get(pivot), surfaceParRef.get(pivot), autres, adresseParRef.get(pivot));
    }
    if (!lien) return null;
    return lien + '&doc=1&docauto=1' + suffixeDossier
      + (lg.volume ? '&vol=' + encodeURIComponent(lg.volume) : '')
      + suffixeBati(refs, batiParRef) + suffixePP(refs, contoursPlan);
  };
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      // --- Onglet : ASSIETTES DE COPROPRIÉTÉ (03/09/2026, addendum v11 § 5) ---
      // Présent seulement si la société détient des lots dont le sol n'est pas
      // à son compte. Les surfaces de cet onglet ne sont PAS dans le total.
      if (assiettes.length) {
        const libelleEtat = {
        ...
        ajouterOnglet(wb, {
          ...
          remplir: (row, a, i) => {
            ...
            const etat = info ? info.etat : 'recherche';
            const unite = (info && info.unite) || [];
            row.getCell(1).value = i + 1;
            row.getCell(2).value = a.codeParcelle;
            row.getCell(3).value = a.commune || '';
            row.getCell(4).value = a.adresse || '';
            row.getCell(5).value = a.nbLots;
            row.getCell(6).value = a.contenance != null ? Number(a.contenance) : '';
            row.getCell(7).value = (libelleEtat[etat] || etat) + (info && info.motif ? ` — ${info.motif}` : '');
            row.getCell(8).value = info && info.syndicat ? info.syndicat.denomination || '' : '';
            row.getCell(9).value = info && info.syndicat
                        ...
                        <div className="px-6 py-3 border-t border-stone-200 bg-white rounded-b-xl">
                          <div className="flex items-center gap-3 flex-wrap text-sm">
                            ...
                            <button onClick={() => genererCarte('ortho')}
                              disabled={!carteRefs.length || !contoursPlan || !carteRefs.some((r) => contoursPlan?.get(r))}
